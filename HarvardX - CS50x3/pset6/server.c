...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int dot_state = false;
            char method[4];
            char request_target[100];
            char protocol[100];
        
            for (i = 0; line[i] != '\0' && i < 4; ++i) {
                method[i] = line[i];
            }
            method[i] = '\0';
            if (!isalpha(method[0])) {
            ...
            if (line[i] == '/') {
                ...
                if (dot_state == true) {
                    ...
                    while ((line[i] != '\0' && i < 100) && !(isspace(line[i]))) {
                        protocol[j] = line[i];
                        ++i;
                        ++j;
                    }
            ...
            if ((qptr = strstr(request_target, "?")) == NULL) {
                query[0] = '\0';
            }
            else {
                for (i = 1, j = 0; qptr[i] != '\0'; ++i, ++j) {
                    query[j] = qptr[i];
                }
                query[j] = '\0';
            }
            ...
            for (i = 0; root[i] != '\0' && i < 200; ++i) {
                path[i] = root[i];
            }
            for (j = 0; request_target[j] != '\0' && i < 200 && request_target[j] != '?'; ++j, ++i) {
           ...
           if (access(path, R_OK) != 0) {
            ...
            for (i = 0, j = 0; path[i] != '\0'; ++i) {     /* Get the index of the last dot in path array */
                if (path[i] == '.') {
                    j = i;
                }
            }
            j++;
            for (i = 0; path[j] != '\0' && j < 100; ++i, ++j) {
                extension[i] = path[j];
            }
            extension[i] = '\0';
            // dynamic content
...
{
    // TODO
    char ext_copy[20];    /* copy the extension contents converting it to lower case */
    int i;
    ...
    for (i = 0; extension[i] != '\0'; ++i) {
        ext_copy[i] = tolower(extension[i]);
    }
    ext_copy[i] = '\0';
    if (strcmp(ext_copy, "css") == 0) {
        return "text/css";
    }
    else if (strcmp(ext_copy, "html") == 0) {
        return "text/html";
    }
    else if (strcmp(ext_copy, "gif") == 0) {
        return "image/gif";
    }
    else if (strcmp(ext_copy, "ico") == 0) {
        return "image/x-icon";
    }
    else if (strcmp(ext_copy, "jpg") == 0) {
        return "image/jpeg";
    }
    else if (strcmp(ext_copy, "js") == 0) {
        return "text/javascript";
    }
    else if (strcmp(ext_copy, "png") == 0) {
        return "image/png";
    }
    else {
        return NULL;
